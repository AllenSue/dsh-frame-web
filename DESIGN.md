...
>
> **frame 本身也不是卡片**：没有自己的底色、没有边框、没有圆角（T27）。它曾经三样都有，后果是两格相邻处出现**两道边**（2px 的缝，还被圆角切成缺口），而且每个占据者周围都围了一圈 frame 的底色——面板自己会画底色，它并不知道自己在一个 frame 里。出厂的外壳一格 chrome 都没有（`AppFrame.module.css`：中央列没有底色也没有边框，左栏是一层 fill 加一条半像素的边），"frame 之间没有间隙"就是照这个来的。焦点仍然要看得出来（`C-x C-d`、`C-x o`、选择器都作用在聚焦的那一格上），所以用**内描边**：画在 frame 里面，既不占位置，也不会自己制造一道缝。
>
> **边界由分隔条那一层画，而且只画一次**（T27 第二轮）。frame 透明之后，"没有边框"和"看不出格与格的分界"是两件事——用户先抱怨缝，再抱怨没有边，答案是同一条：**边界不该由相邻两格各画一道边来表示，而该由那条边界自己表示**。所以每个 `view.dividers` 条目画一根 **1px 的细线**，位置用它自己的 `rect`（零厚度的边界，正是 `dividerRect` 的输出），压在两侧 frame **之上**：不占布局、不留缝、也不会翻倍。抓取条是细线外面一圈更宽的透明盒子，**只在 `movable` 的边界上画**——固定宽度那一列的边上有一条真实的边界（所以有线），但拖动改不动它（所以没有光标、没有抓手）。
