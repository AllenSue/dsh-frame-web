...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function isFixed(state            , childId        )          {
		  return state.layout.nodes[childId]?.kind === 'pane' && !growsIn(state, childId          )
		}
		...
		function resizePane(
		  ...
		  const sizeOf = (childId        )         => parent.sizes[parent.children.indexOf(childId)] ?? 0
		 ...
		 */
		function withoutUnregistered(layout             , types                   )              {
		  /** Whether this pane displays a kind the shell has not registered. */
		  const unregistered = (id        , current             )          => {
		 ...
		 */
		function fixedChild(state            , childId                    )          {
		  if (childId === undefined) return false
		  const node = state.layout.nodes[childId          ]
		  if (node === undefined || node.kind !== 'pane') return false
		  const tabId = node.tabs[0]
		...
		function pickerChoices(source              )                          {
		  return [
		    ...source.contents
		      ...
		      .map((content)               => ({ group: 'open', id: content.id, title: content.title })),
		    ...source.types
		...
		const STARTUP_KEY = 'dsh.frames.startup'
		 ...
		 */
		function readStartup(storage               , key         = STARTUP_KEY)                     {
		 ...
		 * @param key - the entry; defaults to this plugin's.
		 */
		function writeStartup(storage               , name                    , key         = STARTUP_KEY)       {
		 ...
		 */
		function nextStartup(active                    , startup                    )                     {
		 ...
		 * @returns the dialog.
		 */
		function createListDialog(props                 , ...children           )          {
		  ...
		  const onKeyDown = (event   
		    ...
		    const handled = (act            )       => { event.preventDefault(); act() }
		    switch (event.key) {
		...
		function createLayer(controller                                     ) {
		  return function FramesLayer({ renderSlot }   
		    ...
		    const [presetList, setPresetList] = useState                                              (undefined)
		    ...
		    const presetCatalog = presetRows(
		      ...
		      startupPreset(),
		    ).map((row) => ({ group: 'open'         , id: row.name, title: row.label }))
