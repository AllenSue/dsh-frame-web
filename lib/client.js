...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CONVERSATION                      = { id: 'conversation', title: () => 'Conversation' }
		...
		function createController() {
		  ...
		  const measured = ()             => withMeasurements(
		    state,
		  ...
		  let snapshot           = { view: project(state) }
		  ...
		  return {
		    ...
		    split()       { this.run(splitFrame(state, undefined, CONVERSATION.id)) },
		    close()       { this.run(closeFrame(state)) },
		    float()       { this.run(floatFrame(state)) },
		    dock()       { this.run(dockFrame(state)) },
		    focus(direction                                  )       { this.run(moveFocus(state, direction)) },
		    focusPane(paneId        )       { this.run(focusFrame(state, paneId         )) },
		  }
		}
